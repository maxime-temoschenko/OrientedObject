    ...
    public void pickPassenger(Passenger p , boolean seated) throws TransportAccessException{
        ...
        if(seated){
            ...
            if(getRemainingSeating() == 0)
                throw new TransportAccessException("[DENIED ACCES] : " + p.toString() + " Cause : no seating place");
            ++this.occupiedSeating;
        }
        // Standing place
        else{
            if(getRemainingStanding() == 0) // No more standing places
            throw new TransportAccessException("[DENIED ACCES] : " + p.toString() + " Cause : no standing place");
